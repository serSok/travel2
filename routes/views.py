...
def find_routes(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if len(all_ways) == 0:
                messages.error(request, 'Нет маршрутов')
                return render(request, 'routes/home.html', {'form' : form})
            if across_cities_form:
                ...
                if not right_ways:
                    messages.error(request, 'нет через город этоти')
                    return render(request, 'routes/home.html', {'form' : form})
            else:
                right_ways = all_ways
            ...
            for route in right_ways:
                ...
                if total_time <= traveling_time:
                    trains.append(tmp)
            if not trains:
                messages.error(request, 'надо болше времени на путь')
                return render(request, 'routes/home.html', {'form' : form})
...
def add_route(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "Добавлен маршрут")
            return redirect('/')
    else:
        data = request.GET
        if data:
            ...
            route_desc = []
            for tr in qs:
                dsc = '''Поезд {} из {} в {} будет в пути {} ч'''.format(tr.name, tr.from_city, tr.to_city, tr.travel_time)
            ...
            messages.error(request, 'не бывает такого маршрут')
            return redirect('/')
